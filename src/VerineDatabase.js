export class VerineDatabase {
  constructor(name, currentDatabase, type, folder) {
    this.name = name;
  ...
  getNewExerciseOrderAfterId(getNewOrderAfterId) {
    ...
    this.exerciseOrder.forEach((orderElement) => {
      if (orderOfId != undefined) {
        //alle Übungen nach der gesuchten, bekommen eine reihenfolge +1
      ...
      if (orderElement[1] == getNewOrderAfterId) {
        //finde Ordernummer der übergebenen ID
      ...
      if (orderElement[1] == exerciseMoveId) {
        if (moveDirection == "down") {
          if (index < this.exerciseOrder.length - 1) {
            ...
            updateQuery +=
              "UPDATE " + this.exerciseTable + " SET reihenfolge = " + this.exerciseOrder[indexOfNextExerciseOrder][0] + " WHERE id = " + this.exerciseOrder[indexOfNextExerciseOrder][1] + ";";
          }
  ...
  addExercise(newExercise, currentExcersiseId) {
    ...
    let exerciseValues =
      ...
      newExercise.titel +
      '", ' +
      parseInt(newExercise.reihenfolge) +
      ', "' +
    ...
    currentTableColumns.forEach((column) => {
      ...
      newArray.forEach((element) => {
        ...
        if (regexForeignKeyColumnSelf != null) {
          ...
          if (newColumnObject.name == columnWithForeignKey) {
            //Informationen werden nur ergänzt, wenn es sich um die richtige Spalte handelt
